...
/**
 * Checksum-Backfill (Step 038): trägt bei einem alten Backup ohne Prüfsumme eine SHA-256 in die
 * metadata.json nach. OWNER-only, rate-limitiert. Die tar.gz wird NIE verändert; auch archivierte
 * managed Server dürfen nachrüsten. Ergebnis landet als Query-Status in der Backup-Liste.
 */
export async function backfillChecksumAction(formData: FormData): Promise<void> {
  const locale = String(formData.get('locale') ?? 'de');
  const id = String(formData.get('id') ?? '');
  ...
  const rlKey = `backup:backfill:${user.id}`;
  if ((await checkRateLimit(rlKey, CONNECT_RATE_LIMIT)).limited) {
    redirect(`/${locale}/servers/${id}?backups=1&notice=rateLimited`);
  }
  await recordRateLimitHit(rlKey);

  const result = await backfillBackupChecksumForServer(id, user.email, fileName);
  if (result.status === 'notFound') {
    redirect(`/${locale}/servers`);
  }
  redirect(`/${locale}/servers/${id}?backups=1&backfill=${result.status}`);
}
